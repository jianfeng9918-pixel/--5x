...
const managedAssetsDir = path.join(siteRoot, "assets", "managed");
const managedSlotsDir = path.join(managedAssetsDir, "slots");
const managedOverridesDir = path.join(managedAssetsDir, "overrides");
const managedIncomingDir = path.join(managedAssetsDir, "incoming");
const imageSlotsFile = path.join(siteRoot, "image-slots.json");
const imageSlotsGuideFile = path.join(siteRoot, "图片替换对照表.md");
const imageSlotsGuideAliasFile = path.join(siteRoot, "image-slot-guide.md");

const IMAGE_EXTENSIONS = new Set([".png", ".jpg", ".jpeg", ".svg", ".webp", ".gif", ".avif"]);
...
function makeImageSlotId(index) {
  return `IMG-${String(index).padStart(3, "0")}`;
}

function parseImageSlotNumber(slotId) {
  const match = String(slotId || "").match(/(\d+)/);
  return match ? Number(match[1]) : 0;
}

function normalizeIncomingSlotToken(filePath) {
  const baseName = path.basename(filePath, path.extname(filePath)).toUpperCase();
  if (/^IMG-\d+$/.test(baseName)) {
    return baseName;
  }
  if (/^\d+$/.test(baseName)) {
    return makeImageSlotId(Number(baseName));
  }
  return baseName;
}

function normalizeSlotAlias(value) {
  return String(value || "")
    .replace(/\.[^.]+$/i, "")
    .toUpperCase()
    .replace(/[^A-Z0-9\u4E00-\u9FFF]+/g, "");
...
function collectSlotAliases(slotLike) {
  const aliases = new Set();
  const slotId = String(slotLike?.id || "").toUpperCase();
  const label = String(slotLike?.label || "");
  const sourcePath = String(slotLike?.sourcePath || "");
  const sourceRelativePath = String(slotLike?.sourceRelativePath || "");

  if (slotId) {
    aliases.add(slotId);
    aliases.add(slotId.replace(/^IMG-/, ""));
  }

  const labelBase = path.basename(label || "");
  const sourceBase = path.basename(sourcePath || "");
  const relativeBase = path.basename(sourceRelativePath || "");

  [labelBase, sourceBase, relativeBase].forEach((candidate) => {
    if (!candidate) {
      return;
    }
    aliases.add(path.basename(candidate, path.extname(candidate)).toUpperCase());
    const loose = normalizeSlotAlias(candidate);
    if (loose) {
      aliases.add(loose);
    }
  });
...
function collectIncomingAliases(filePath) {
  const aliases = new Set();
  const direct = normalizeIncomingSlotToken(filePath);
  if (direct) {
    aliases.add(direct);
  }
  const loose = normalizeSlotAlias(path.basename(filePath));
  if (loose) {
    aliases.add(loose);
  }
  return Array.from(aliases).filter(Boolean);
...
async function loadImageSlotsRegistry() {
  if (!(await pathExists(imageSlotsFile))) {
    return { generatedAt: null, slots: [] };
  }

  try {
    const raw = JSON.parse(await fs.readFile(imageSlotsFile, "utf8"));
    if (Array.isArray(raw)) {
      return { generatedAt: null, slots: raw };
    }
    if (raw && Array.isArray(raw.slots)) {
      return raw;
    }
  } catch (error) {
    console.warn(`Unable to parse image slot registry: ${imageSlotsFile}`);
  }
...
function normalizeMarkdownAssetPaths(markdown) {
  return markdown
    .replace(/(?:\.\.\/)+_本地正式版网站\/assets\//g, "./assets/")
    .replace(/_本地正式版网站\/assets\//g, "./assets/");
}
...
async function applyImageSlotRegistry(manifest) {
  await fs.mkdir(managedSlotsDir, { recursive: true });
  await fs.mkdir(managedOverridesDir, { recursive: true });
  await fs.mkdir(managedIncomingDir, { recursive: true });

  const existingRegistry = await loadImageSlotsRegistry();
  const existingBySource = new Map();
  const usedNumbers = new Set();

  for (const slot of existingRegistry.slots || []) {
    if (slot?.sourcePath) {
      existingBySource.set(normalizeAssetReference(slot.sourcePath), slot);
    }
    const slotNumber = parseImageSlotNumber(slot?.id);
    if (slotNumber) {
      usedNumbers.add(slotNumber);
    }
  }

  const discoveredImages = (await walkFiles(assetsDir))
    .filter(isImageFile)
    .sort((a, b) => toPosix(path.relative(siteRoot, a)).localeCompare(toPosix(path.relative(siteRoot, b)), "zh-CN"));

  const incomingFiles = (await walkFiles(managedIncomingDir))
    .filter(isImageFile);
  const incomingByToken = new Map();
  for (const filePath of incomingFiles) {
    for (const token of collectIncomingAliases(filePath)) {
      if (!incomingByToken.has(token)) {
        incomingByToken.set(token, filePath);
      }
    }
  }

  const existingOverrideFiles = (await walkFiles(managedOverridesDir))
    .filter(isImageFile);
  const existingOverrideByToken = new Map();
  for (const filePath of existingOverrideFiles) {
    for (const token of collectIncomingAliases(filePath)) {
      if (!existingOverrideByToken.has(token)) {
        existingOverrideByToken.set(token, filePath);
      }
    }
  }

  let nextSlotNumber = 1;
  const allocateSlotId = () => {
    while (usedNumbers.has(nextSlotNumber)) {
      nextSlotNumber += 1;
    }
    usedNumbers.add(nextSlotNumber);
    return makeImageSlotId(nextSlotNumber);
  };

  const slots = [];

  for (const imagePath of discoveredImages) {
    const sourcePath = normalizeAssetReference(toPublicPath(imagePath));
    const previous = existingBySource.get(sourcePath);
    const slotId = previous?.id || allocateSlotId();
    const sourceExt = path.extname(imagePath).toLowerCase() || ".png";
    const slotFileName = `${slotId}${sourceExt}`;
    const slotOutputPath = path.join(managedSlotsDir, slotFileName);
    await fs.copyFile(imagePath, slotOutputPath);

    const slotAliases = collectSlotAliases({
      id: slotId,
      label: previous?.label || path.basename(imagePath),
      sourcePath,
      sourceRelativePath: toPosix(path.relative(siteRoot, imagePath))
    });
    const incomingOverride = slotAliases
      .map((alias) => incomingByToken.get(alias))
      .find(Boolean);
    let overrideOutputPath = null;

    if (incomingOverride) {
      overrideOutputPath = path.join(managedOverridesDir, `${slotId}${path.extname(incomingOverride).toLowerCase() || sourceExt}`);
      await fs.copyFile(incomingOverride, overrideOutputPath);
      for (const alias of slotAliases) {
        existingOverrideByToken.set(alias, overrideOutputPath);
      }
    } else {
      overrideOutputPath = slotAliases
        .map((alias) => existingOverrideByToken.get(alias))
        .find(Boolean) || null;
    }

    const slotRecord = {
      id: slotId,
      label: previous?.label || humanizeSegment(path.basename(imagePath)),
      sourcePath,
      sourceRelativePath: toPosix(path.relative(siteRoot, imagePath)),
      defaultPath: toPublicPath(slotOutputPath),
      overridePath: overrideOutputPath ? toPublicPath(overrideOutputPath) : "",
      currentPath: overrideOutputPath ? toPublicPath(overrideOutputPath) : toPublicPath(slotOutputPath),
      recommendedRatio: previous?.recommendedRatio || "auto",
      usage: previous?.usage || "",
      pageHints: previous?.pageHints || []
    };

    slots.push(slotRecord);
  }

  const slotBySourcePath = new Map(slots.map((slot) => [normalizeAssetReference(slot.sourcePath), slot]));
  for (const [assetId, asset] of Object.entries(manifest)) {
    const slot = slotBySourcePath.get(normalizeAssetReference(asset.publicPath));
    if (!slot) {
      continue;
    }
    manifest[assetId] = {
      ...
      overridePath: slot.overridePath,
  ...
  await fs.writeFile(imageSlotsFile, JSON.stringify(registry, null, 2), "utf8");
  const guideLines = [
    "# 图片替换对照表",
    "",
    "## 怎么换图",
    "1. 在 `assets/managed/incoming` 里放入新图。",
    "2. 文件名可以用 `IMG-001.png` 这种编号，也可以直接用原图名字，例如 `brand-space.png`。",
    "3. 运行 `node build.mjs` 重建站点。",
    "4. 构建后会自动覆盖到对应图片槽位。",
    "",
    "## 当前图片槽位",
    "",
  ...
  for (const slot of slots) {
    guideLines.push(`| ${slot.id} | ${slot.label} | \`${slot.defaultPath}\` | \`${slot.currentPath}\` |`);
  }
  guideLines.push("");
  await fs.writeFile(imageSlotsGuideFile, guideLines.join("\n"), "utf8");
  await fs.writeFile(imageSlotsGuideAliasFile, guideLines.join("\n"), "utf8");
...
function buildSections(docs) {
  ...
  const coCreateWorkshopTypesId = findDocId(
    docs,
    (doc) =>
      doc.relativePath.startsWith(`${coCreateTop}/${coCreateFoundationFolder}/`) &&
      pathBase(doc.relativePath).startsWith("59_")
  );
  const coCreateNeedsWorkshopId = findDocId(
    docs,
    (doc) =>
      doc.relativePath.startsWith(`${coCreateTop}/${coCreateFoundationFolder}/`) &&
      pathBase(doc.relativePath).startsWith("60_")
  );
  const coCreateProductWorkshopId = findDocId(
    docs,
    (doc) =>
      doc.relativePath.startsWith(`${coCreateTop}/${coCreateFoundationFolder}/`) &&
      pathBase(doc.relativePath).startsWith("61_")
  );
  ...
  const coCreateJourneyGroups = [
    ...
    },
    {
      id: "co-create-types",
      title: "再看共创有哪些类型",
      description: "先把需求工作坊、产品工作坊、品牌定义工作坊和复盘工作坊分清楚，再决定自己该走哪条线。",
      cta: "看工作坊类型",
      leadDocId: coCreateWorkshopTypesId || coCreateDiscoveryWorkshopId,
      docIds: [coCreateWorkshopTypesId, coCreateDiscoveryWorkshopId, coCreateDefinitionWorkshopId, coCreateProductWorkshopId].filter(Boolean)
    },
    {
      id: "co-create-workshops",
      title: "再看工作坊如何推进",
      description: "围绕需求工作坊、产品工作坊和发现定义两类关键工作坊，理解问题如何被识别、被聚焦、被变成行动。",
      cta: "看工作坊流程",
      leadDocId: coCreateNeedsWorkshopId || coCreateProductWorkshopId || coCreateDiscoveryWorkshopId,
      docIds: [coCreateNeedsWorkshopId, coCreateProductWorkshopId, coCreateDiscoveryWorkshopId, coCreateDefinitionWorkshopId, coCreateWorkshopId].filter(Boolean)
    },
    {
...
async function main() {
  ...
  for (const file of markdownFiles) {
    ...
    const normalizedMarkdown = normalizeMarkdownAssetPaths(rawMarkdown);
    const rewrittenMarkdown = rewriteMarkdownLinks(normalizedMarkdown, file, slugMap);
  ...
  const imageSlots = await applyImageSlotRegistry(assets);
